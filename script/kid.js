...
fetch('product.json')
  ...
  .then(function (data) {
    data.forEach(element => {
      if (element.category.toUpperCase() == `${headerval[0].toUpperCase()}`) {
...
function filter(id) {
  ...
  fetch('product.json')
    ...
    .then(function (data) {
      data.forEach(element => {
        if ((element.category.toUpperCase() == `${headerval[0].toUpperCase()}` && element.subCategory.toUpperCase() == `${id.toUpperCase()}`) || ('all' == `${id}` && element.category.toUpperCase() == `${headerval[0].toUpperCase()}`)) {
          searchContentContainer.innerHTML = searchContentContainer.innerHTML + `
               <div class="product-box" onclick="redirectProductInfo(this)" style="display: flex;">
               <img src="${element.picture}" class="product-image">
               <span class="product-name">${element.name}</span>
